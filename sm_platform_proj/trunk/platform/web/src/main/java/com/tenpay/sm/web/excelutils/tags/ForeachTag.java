...
import com.tenpay.sm.web.excelutils.WorkbookUtils;
...
public class ForeachTag implements ITag {
	...
	public int[] parseTag(Object context, HSSFSheet sheet, HSSFRow curRow,
		...
		/*
		 * if (null == collection) { return new int[] { 0, 0, 1 }; }
		 */
		...
		if (null != iterator) {
			...
			// 检查当前sheet是否够大。sheet总行数<=65536
			int size = ExcelParser.getCollectionSize(collection);
			if (shiftNum * size + forstart > 65500) {
				// shift the #foreach #end block
				sheet.shiftRows(forstart, forend, shiftNum, true, true);
				// copy the body of #foreach #end block
				WorkbookUtils.copyRow(sheet, forstart + shiftNum + 1, forstart,
						shiftNum);
				HSSFRow row = WorkbookUtils.getRow(forstart, sheet);
				short lastCellNum = row.getLastCellNum();
				short firstCellNum = row.getFirstCellNum();
				for (short i = lastCellNum; i > firstCellNum; i--) {
					row.removeCell(row.getCell(i));
				}
				HSSFCell cell = WorkbookUtils.getCell(row, firstCellNum);
				cell.setEncoding(HSSFWorkbook.ENCODING_UTF_16);
				cell.setCellValue("数据长度超限! Data is too long!");
				...
				while (iterator.hasNext()) {
					...
					ExcelUtils.addValue(context, property + "Id", new Integer(
							propertyId));
					// Index start with 1
					ExcelUtils.addValue(context, property + "Index",
							new Integer(propertyId + 1));

					// shift the #foreach #end block
					sheet.shiftRows(forstart, sheet.getLastRowNum(), shiftNum,
							true, true);
					// copy the body of #foreach #end block
					WorkbookUtils.copyRow(sheet, forstart + shiftNum + 1,
							forstart, shiftNum);
					// parse
					shift = ExcelParser.parse(context, sheet, forstart,
							forstart + shiftNum - 1);
		...
		sheet.shiftRows(forend + 1, sheet.getLastRowNum(),
				-(forend - forstart + 1), true, true);
