 ...
 */
@Aspect
public class AuthLocalByTenpayOrMchAspect implements Ordered {
	private AuthDispatcherFacade authDispatcher;

	/***
	 * 　　 * 切点 　　
	 */
	@Pointcut("@annotation(com.tenpay.common.aop.annotation.AuthLocalByTenpayOrMch)")
	...
	public Object auth(ProceedingJoinPoint joinPoint) throws Throwable {
		...
		if (authDispatcher
				.commonAuth(AuthDispatcherFacade.LOCAL_BY_TENPAY_MASK, 0, args) > 0) {
			return joinPoint.proceed();
		} else if(authDispatcher
				.commonAuth(AuthDispatcherFacade.LOCAL_BY_MCH_MASK, 0, args) > 0) {
			return joinPoint.proceed();
		}else {
